...
import { createNewNode, createAlternativeNodes, createAlternativeEdges, createInputNode, handleInsertOnCanvaAtPosition } from '../utils/canvasUtils';
...
interface UseCanvasHandlersProps {
  ...
  isEditMode: boolean;
  editingElementModal: { isOpen: boolean; surveyElement: SurveyElementDto | null };
...
export function useCanvasHandlers({
  ...
  const handleModalConfirm = useCallback((modalData: {
    titulo: string;
    tipo: string;
    alternativas: string[];
  }) => {
    ...
    if (selectedNodeId && isEditMode) {
      ...
      if (modalData.tipo === 'MultiplaEscolha' || modalData.tipo === 'Alternativa') {
        ...
        const newEdges = createAlternativeEdges(selectedNodeId, newNodes);
      ...
      const newNode = createNewNode(
        ...
        modalData.titulo,
        nodeType,
        modalData.tipo === 'MultiplaEscolha' || modalData.tipo === 'Alternativa'
          ? modalData.alternativas.length
      ...
      if (modalData.tipo === 'MultiplaEscolha' || modalData.tipo === 'Alternativa') {
        ...
        const alternativeEdges = createAlternativeEdges(newNodeId, alternativeNodes);
  ...
  }, [nodes, edges, setNodes, setEdges, saveToLocalStorage, handleNodeDelete, handleNodeDoubleClick]);
  ...
  const handleInsertOnCanva = useCallback((element: SurveyElementDto) => {
