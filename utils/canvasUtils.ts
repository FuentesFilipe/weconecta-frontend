...
import { SurveyElementDto, SurveyElementType } from '@/dtos/SurveysElementsDto';

const mapNodeTypeToSurveyType = (
    t: 'mensagem' | 'alternativa' | 'input' | 'fim',
): SurveyElementType => {
    switch (t) {
        case 'input':
...
export const ensureNodeFunctions = (
    ...
    handleEditElement: (el: SurveyElementDto) => void,
) => {
    return nodeList.map((node: any) => {
        const element: SurveyElementDto | undefined =
            node.data?.surveyElement ||
            (node.data?.surveyElementId
                ? {
                      id: Number(node.data.surveyElementId),
                      description: node.data.label,
                      type: mapNodeTypeToSurveyType(node.data.type),
                      options: [],
                      deletedAt: null,
                  }
                : undefined);
...
export const createAlternativeEdges = (
    parentNodeId: string,
    alternativeNodes: any[],
) => {
